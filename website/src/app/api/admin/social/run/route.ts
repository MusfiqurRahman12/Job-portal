...
export async function POST(req: Request) {
  try {
    ...
    if (id) {
      ...
      if (error) {
        return NextResponse.json({ error: `Failed to fetch post: ${error.message}` }, { status: 404 });
      }
      postsToProcess = [data];
    } else {
      ...
      if (error) {
        return NextResponse.json({ error: `Failed to query scheduled posts: ${error.message}` }, { status: 500 });
      }
      postsToProcess = data || [];
    }

    if (postsToProcess.length === 0) {
      return NextResponse.json({ message: 'No scheduled posts ready for execution.' });
    }
