...
// AssignDynamicImage sets a unique image from loremflickr using a specific keyword generated by the AI
func (n *News) AssignDynamicImage(keyword string) {
	if n.Image != "" {
		return
	}
	// Generate random number to bypass browser caching and ensure uniqueness
	timestamp := time.Now().UnixNano()
	// Clean the keyword
	kw := strings.ReplaceAll(strings.ToLower(keyword), " ", ",")
	n.Image = fmt.Sprintf("https://loremflickr.com/1200/600/%s?lock=%d", kw, timestamp%10000)
}
